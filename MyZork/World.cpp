...
World::World() {
	...
	Room* _orcRoom = new Room("OrcRoom", "You are in a room where someone has lived for a long time.");
	...
	entities.push_back(_orcRoom);
	entities.push_back(_treasureChamber);

	// CREATURES
	Creature* _orc = new Creature("Orc", "A soldier orc, you may win him with a good weapon.", 20, 8, _orcRoom);
	...
	Item* _goldenKey = new Item("GoldenKey", "A golden key.", key, _orcRoom);
	...
	Exit* _door = new Exit("Door", "You see a wooden door with no knob.", "south", "north", _darkRoom, _orcRoom);
